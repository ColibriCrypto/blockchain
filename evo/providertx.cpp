...
bool CheckProRegTx(const CTransaction& tx, const CBlockIndex* pindexPrev, CValidationState& state)
{
    ...
    if (pl.collateralOutpoint.hash.IsNull()) {
        ...
        // the   exists and it is unspent.
           ;
        if (!GetUTXO (pl.collateralOutpoint,  )) {
...
bool CheckProUpRegTx(const CTransaction& tx, const CBlockIndex* pindexPrev, CValidationState& state)
{
    ...
    if (pindexPrev) {
        ...
        if (payoutDest == CTxDestination(dmn->pdmnState->keyIDOwner)) {
           ...
           ;
        if (!GetUTXO (dmn->collateralOutpoint,  )) {
        ...
        CTxDestination collateralTxDest;
        if (!ExtractDestination( .out.scriptPubKey, collateralTxDest)) {
