...
int64_t UpdateTime(CBlockHeader* pblock, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    // Updating time can change work required on  :
...
static CMutableTransaction New base(const int nHeight, const CScript* pScriptPubKey = nullptr)
{
    CMutableTransaction tx base;
    tx base.vout.emplace_back();
    tx base.vout[0].SetEmpty();
    if (pScriptPubKey) tx base.vout[0].scriptPubKey = *pScriptPubKey;
    tx base.vin.emplace_back();
    tx base.vin[0].scriptSig = CScript() << nHeight << OP_0;
    return tx base;
}

bool SolveProofOfStake(CBlock* pblock, CBlockIndex* pindexPrev, CWallet* pwallet,
                       std::vector<CStakeableOutput>* available s, bool stopPoSOnNewBlock)
{
    ...
    // Sync wallet before create  stake
    pwallet->BlockUntilSyncedToCurrentChain();

    CMutableTransaction tx Stake;
    int64_t nTxNewTime = 0;
    if (!pwallet->Create Stake(pindexPrev,
                                  pblock->nBits,
                                  tx Stake,
                                  nTxNewTime,
                                  available s,
    ...
    // Create  base tx and add masternode/budget payments
    CMutableTransaction tx base = New base(pindexPrev->nHeight + 1);
    FillBlockPayee(tx base, tx Stake, pindexPrev, true);

    // Sign  stake
    if (!pwallet->Sign Stake(tx Stake)) {
        const COutPoint& stakeIn = tx Stake.vin[0].prevout;
        return error("Unable to sign  stake with input %s-%d", stakeIn.hash.ToString(), stakeIn.n);
    }

    pblock->vtx.emplace_back(MakeTransactionRef(tx base));
    pblock->vtx.emplace_back(MakeTransactionRef(tx Stake));
    pblock->nTime = nTxNewTime;
    return true;
}

CMutableTransaction Create baseTx(const CScript& scriptPubKeyIn, CBlockIndex* pindexPrev)
{
    assert(pindexPrev);
    const int nHeight = pindexPrev->nHeight + 1;

    // Create  base tx
    CMutableTransaction tx base = New base(nHeight, &scriptPubKeyIn);

    //Masternode and general budget payments
    CMutableTransaction txDummy;    // POW blocks have no  stake
    FillBlockPayee(tx base, txDummy, pindexPrev, false);

    // If no payee was detected, then the whole block value goes to the first output.
    if (tx base.vout.size() == 1) {
        tx base.vout[0].nValue = GetBlockValue(nHeight);
    }

    return tx base;
}

bool Create baseTx(CBlock* pblock, const CScript& scriptPubKeyIn, CBlockIndex* pindexPrev)
{
    pblock->vtx.emplace_back(MakeTransactionRef(Create baseTx(scriptPubKeyIn, pindexPrev)));
    return true;
}
...
void BlockAssembler::resetBlock()
{
    ...
    // Reserve space for  base tx
    nBlockSize = 1000;
    nBlockSigOps = 100;

    // These counters do not include  base tx
    nBlockTx = 0;
    nFees = 0;
}
                                               ...
                                               bool stopPoSOnNewBlock)
{
    ...
    // Depending on the tip height, try to find a  stake who solves the block or create a  base tx.
    if (!(fProofOfStake ? SolveProofOfStake(pblock, pindexPrev, pwallet, available s, stopPoSOnNewBlock)
                        : Create baseTx(pblock, scriptPubKeyIn, pindexPrev))) {
        return nullptr;
    }
    ...
    if (!fProofOfStake) {
        //  base can get the fees.
        CMutableTransaction tx base(*pblock->vtx[0]);
        tx base.vout[0].nValue += nFees;
        pblock->vtx[0] = MakeTransactionRef(tx base);
...
uint256 CalculateSaplingTreeRoot(CBlock* pblock, int nHeight, const CChainParams& chainparams)
{
    if (NetworkUpgradeActive(nHeight, chainparams.GetConsensus(), Consensus::UPGRADE_V5_0)) {
        ...
        assert(p sTip->GetSaplingAnchorAt(p sTip->GetBestAnchor(), sapling_tree));
...
void IncrementExtraNonce(std::shared_ptr<CBlock>& pblock, int nHeight, unsigned int& nExtraNonce)
{
    ...
    ++nExtraNonce;
    CMutableTransaction tx base(*pblock->vtx[0]);
    tx base.vin[0].scriptSig = (CScript() << nHeight << CScriptNum(nExtraNonce)) +  BASE_FLAGS;
    assert(tx base.vin[0].scriptSig.size() <= 100);
