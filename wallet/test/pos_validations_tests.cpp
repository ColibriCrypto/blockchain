...
BOOST_FIXTURE_TEST_CASE( stake_tests, TestPoSChainSetup)
{
    ...
    BOOST_CHECK(pwalletMain->Stakeable s(&available s));
    ...
    // Add a second input to a  stake
    CMutableTransaction mtx(*pblock->vtx[1]);
    const CStakeableOutput& in2 = available s.back();
    available s.pop_back();
...
CTransaction CreateAndCommitTx(CWallet* pwalletMain, const CTxDestination& dest, CAmount destValue, C Control*  Control = nullptr)
{
    CTransactionRef txNew;
    CReserveKey reservekey(pwalletMain);
    CAmount nFeeRet = 0;
    std::string strFailReason;
    // The minimum depth (100) required to spend  base outputs is calculated from the current chain tip.
...
static bool IsSpentOnFork(const COutput&  , std::initializer_list<std::shared_ptr<CBlock>> forkchain = {})
{
    for (const auto& block : forkchain) {
        const auto& usedOutput = block->vtx[1]->vin.at(0).prevout;
        if ( .tx->GetHash() == usedOutput.hash &&  .i == (int)usedOutput.n) {
            // spent on fork
            return true;
        }
    }
                                            ...
                                            std::initializer_list<std::shared_ptr<CBlock>> forkchain = {})
{
    ...
    //  stake inputs on the previous blocks of the parallel chain so they
    // are not used again.
...
static COutput GetUnspent (CWallet* pwallet, std::initializer_list<std::shared_ptr<CBlock>> forkchain = {})
{
    std::vector<COutput> available s;
    CWallet::Available sFilter  sFilter;
     sFilter.minDepth = 120;
    BOOST_CHECK(pwallet->Available s(&available s, nullptr,  sFilter));
    for (const auto&   : available s) {
        if (!IsSpentOnFork( , forkchain)) {
            return  ;
        }
    }
    throw std::runtime_error("Unspent   not found");
}
...
BOOST_FIXTURE_TEST_CASE(created_on_fork_tests, TestPoSChainSetup)
{
    ...
    // Tests:
    // 1)  s created in D1 and spent in E1. --> should pass
    // 2)  s created in E, being spent in D4 --> should fail.
    // 3)  s created and spent in E2, being double spent in F2. --> should fail
    // 4)  s created in D and spent in E3. --> should fail
    // 5)  s create in D, spent in F and then double spent in F3. --> should fail
    // 6)  s created in G and G3, being spent in H and H3 --> should pass.
    // 7) use  stake on different chains --> should pass.

    // Let's create block C with a valid cTx
    auto cTx = CreateAndCommitTx(pwalletMain.get(), *pwalletMain->getNewAddress("").getObjResult(), 249 *  );
    const auto& cTx_out = GetOutpointWithAmount(cTx, 249 *  );
    WITH_LOCK(pwalletMain->cs_wallet, pwalletMain->Lock (cTx_out));
    ...
    auto dTx = CreateAndCommitTx(pwalletMain.get(), *pwalletMain->getNewAddress("").getObjResult(), 249 *  );
    auto dTxOutPoint = GetOutpointWithAmount(dTx, 249 *  );
    WITH_LOCK(pwalletMain->cs_wallet, pwalletMain->Lock (dTxOutPoint));
    ...
    auto d1Tx = CreateAndCommitTx(pwalletMain.get(), *dest, 200 *  );
    ...
    // Ensure that the   does not exist in the main chain
    const  & utxo = p sTip->Access (COutPoint(d1Tx.GetHash(), 0));
    BOOST_CHECK(utxo.out.IsNull());

    // Create valid block E
    auto eTx = CreateAndCommitTx(pwalletMain.get(), *dest, 200 *  );
    std::shared_ptr<CBlock> pblockE = CreateBlockInternal(pwalletMain.get(), {eTx});
    BOOST_CHECK(ProcessNewBlock(pblockE, nullptr));

    // #################################################
    // ### 1) ->  s created in D' and spent in E' ###
    // #################################################

    // Create tx spending the previously created tx on the forked chain
    C Control  Control;
     Control.fAllowOtherInputs = false;
     Control.Select(COutPoint(d1Tx.GetHash(), 0), d1Tx.vout[0].nValue);
    auto e1Tx = CreateAndCommitTx(pwalletMain.get(), *dest, d1Tx.vout[0].nValue - 0.1 *  , & Control);
    ...
    // #################################################################
     ...
     Control.UnSelectAll();
     Control.Select(GetOutpointWithAmount(eTx, 200 *  ), 200 *  );
     Control.fAllowOtherInputs = false;
    auto D4_tx1 = CreateAndCommitTx(pwalletMain.get(), *dest, 199 *  , & Control);
    ...
    // #####################################################################
    // ### 3) ->  s created and spent in E2, being double spent in F2 ###
    // #####################################################################

    // Create block E2 with E2_tx1 and E2_tx2. Where E2_tx2 is spending the outputs of E2_tx1
    C Control  ControlE2;
     ControlE2.Select(cTx_out, 249 *  );
    auto E2_tx1 = CreateAndCommitTx(pwalletMain.get(), *dest, 200 *  , & ControlE2);

     Control.UnSelectAll();
     Control.Select(GetOutpointWithAmount(E2_tx1, 200 *  ), 200 *  );
     Control.fAllowOtherInputs = false;
    auto E2_tx2 = CreateAndCommitTx(pwalletMain.get(), *dest, 199 *  , & Control);
    ...
    auto F2_tx1 = CreateAndCommitTx(pwalletMain.get(), *dest, 199 *  , & Control);
    ...
    // #############################################
    // ### 4)  s created in D and spent in E3 ###
    // #############################################
    ...
    // Now let's try to spend the  s created in D in E3
     Control.UnSelectAll();
     Control.Select(dTxOutPoint, 249 *  );
     Control.fAllowOtherInputs = false;
    auto E3_tx1 = CreateAndCommitTx(pwalletMain.get(), *dest, 200 *  , & Control);
    ...
    // ####################################################################
    // ### 5)  s create in D, spent in F and then double spent in F3 ###
    // ####################################################################

    // Create valid block F spending the  s created in D
    ...
    // First create new  s in G
    // select an input that is not already spent in D3 or E3 (since we want to spend it also in G3)
    const COutput& input = GetUnspent (pwalletMain.get(), {pblockD3, pblockE3});

     Control.UnSelectAll();
     Control.Select(COutPoint(input.tx->GetHash(), input.i), input.Value());
     Control.fAllowOtherInputs = false;

    dest = pwalletMain->getNewAddress("").getObjResult();
    auto gTx = CreateAndCommitTx(pwalletMain.get(), *dest, 200 *  , & Control);
    auto gOut = GetOutpointWithAmount(gTx, 200 *  );
    std::shared_ptr<CBlock> pblockG = CreateBlockInternal(pwalletMain.get(), {gTx});
    BOOST_CHECK(ProcessNewBlock(pblockG, nullptr));

    // Now create the same   in G3
    ...
    // Now spend the   in both, H and H3
     Control.UnSelectAll();
     Control.Select(gOut, 200 *  );
     Control.fAllowOtherInputs = false;
    auto hTx = CreateAndCommitTx(pwalletMain.get(), *dest, 199 *  , & Control);
    ...
    // #################################################################################
    // ### 7) Now try to use the same  stake on different chains --> should pass. ###
    // #################################################################################

    // Take I3  stake and use it for block I, changing its hash adding a new tx
    std::shared_ptr<CBlock> pblockI = std::make_shared<CBlock>(*pblockI3);
    auto iTx = CreateAndCommitTx(pwalletMain.get(), *dest, 1 *  );
