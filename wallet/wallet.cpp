...
/**
 * Fees smaller than this (in uclbi) are considered zero fee (for transaction creation)
 * We are ~100 times smaller then CLBI now (  06-23), set minTxFee 10 times higher
 * so it's still 10 times lower comparing to CLBI.
...
std::set<uint256> CWallet::GetConflicts(const uint256& txid) const
{
    ...
    for (const CTxIn& txin : wtx.tx->vin) {
        if (mapTxSpends.count(txin.prevout) <= 1 || wtx.tx->HasZero SpendInputs())
...
bool CWallet::IsSpent(const COutPoint& outpoint) const
{
    ...
    for (TxSpends::const_iterator it = range.first; it != range.second; ++it) {
        ...
        if (mit != mapWallet.end()) {
            ...
            // not in mempool txes can spend  s only if not  stakes
            const bool fConflicted stake = fConflicted && mit->second.Is Stake();
            if (nDepth > 0  || (nDepth == 0 && !mit->second.isAbandoned() && !fConflicted stake) )
                return true; // Spent
        }
    }
    return false;
...
void CWallet::AddToSpends(const uint256& wtxid)
{
    ...
    if (thisTx.Is Base()) //  bases don't spend anything!
        return;
...
bool CWallet::AddToWalletIfInvolvingMe(const CTransactionRef& ptx, const CWalletTx::Confirmation& confirm, bool fUpdate)
{
    const CTransaction& tx = *ptx;
    {
        ...
        if (!confirm.hashBlock.IsNull() && !tx.HasZero SpendInputs() && !tx.Is Base()) {
        ...
        // If this is a ProRegTx and the wallet owns the collateral, lock the corresponding  
...
void CWallet::TransactionRemovedFromMempool(const CTransactionRef &ptx, MemPoolRemovalReason reason) {
    ...
    if (reason == MemPoolRemovalReason::CONFLICT) {
        ...
        //    https://github.com/CLBI/CLBI/pull/18600#discussion_r420215993.
        ...
        //    https://github.com/CLBI/CLBI/pull/9371 and the prior sync
        ...
        // https://github.com/CLBI-core/CLBI-devwiki/wiki/Wallet-Transaction-Conflict-Tracking
...
{
    {
        ...
        if (isSaplingActive) {
            assert(p sTip->GetSaplingAnchorAt(pprev->hashFinalSaplingRoot, oldSaplingTree));
        } else {
            assert(p sTip->GetSaplingAnchorAt(SaplingMerkleTree::empty_root(), oldSaplingTree));
        }
...
CAmount CWalletTx::GetAvailableCredit(bool fUseCache, const isminefilter& filter) const
{
    ...
    // Must wait until  base/ stake is safely deep enough in the chain before valuing it
...
// Return sum of locked  s
CAmount CWalletTx::GetLockedCredit() const
{
    if (pwallet == 0)
        return 0;

    // Must wait until  base is safely deep enough in the chain before valuing it
    if (Is Base() && GetBlocksToMaturity() > 0)
        return 0;
    ...
    for (unsigned int i = 0; i < tx->vout.size(); i++) {
        ...
        // Add locked  s
        if (pwallet->IsLocked (hashTx, i)) {
            nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE_ALL);
        }

        // Add masternode collaterals which are handled like locked  s
    ...
    const isminefilter& filter) const
{
    ...
    // Sent/received.
    bool hasZero Spends = tx->HasZero SpendInputs();
    for (unsigned int i = 0; i < tx->vout.size(); ++i) {
        const CTxOut& txout = tx->vout[i];
        ...
        if (nDebit > 0) {
            ...
            if (pwallet->IsChange(txout))
                continue;
        } else if (!(fIsMine & filter) && !hasZero Spends)
            continue;

        // In either case, we need to get the destination address
        const bool fColdStake = (filter & ISMINE_COLD);
        CTxDestination address;
        if (txout.IsZero Mint()) {
            ...
            if (ReadBlockFromDisk(block, pindex)) {
                ...
                if (pindex->pprev) {
                    if (Params().GetConsensus().NetworkUpgradeActive(pindex->pprev->nHeight, Consensus::UPGRADE_V5_0)) {
                        SaplingMerkleTree saplingTree;
                        assert(p sTip->GetSaplingAnchorAt(pindex->pprev->hashFinalSaplingRoot, saplingTree));
...
void CWallet::ReacceptWalletTransactions(bool fFirstLoad)
{
    ...
    for (std::pair<const uint256, CWalletTx>& item: mapWallet) {
        ...
        if (!wtx.Is Base() && !wtx.Is Stake() && nDepth == 0  && !wtx.isAbandoned()) {
...
{
    return loopTxsBalance([filter, useCache, minDepth](const uint256& id, const CWalletTx& p , CAmount& nTotal){
        bool fConflicted;
        int depth;
        if (p .IsTrusted(depth, fConflicted) && depth >= minDepth) {
            nTotal += p .GetAvailableCredit(useCache, filter);
        }
    });
}
...
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& p , CAmount& nTotal) {
        if (p .tx->HasP2CSOutputs() && p .IsTrusted())
            nTotal += p .GetColdStakingCredit();
    });
}
...
CAmount CWallet::GetStakingBalance(const bool fIncludeColdStaking) const
{
    return std::max(CAmount(0), loopTxsBalance(
            [fIncludeColdStaking](const uint256& id, const CWalletTx& p , CAmount& nTotal) {
        if (p .IsTrusted() && p .GetDepthInMainChain() >= Params().GetConsensus().nStakeMinDepth) {
            nTotal += p .GetAvailableCredit();       // available  s
            nTotal -= p .GetStakeDelegationCredit(); // minus delegated  s, if any
            nTotal -= p .GetLockedCredit();          // minus locked  s, if any
            if (fIncludeColdStaking)
                nTotal += p .GetColdStakingCredit(); // plus cold  s, if any and if requested
        }
    }));
}
...
CAmount CWallet::GetDelegatedBalance() const
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& p , CAmount& nTotal) {
            if (p .tx->HasP2CSOutputs() && p .IsTrusted())
                nTotal += p .GetStakeDelegationCredit();
    });
}

CAmount CWallet::GetLocked s() const
{
    if (fLiteMode) return 0;

    LOCK(cs_wallet);
    if (setLocked s.empty()) return 0;

    CAmount ret = 0;
    for (const auto&   : setLocked s) {
        auto it = mapWallet.find( .hash);
        if (it != mapWallet.end()) {
            const CWalletTx& p  = it->second;
            if (p .IsTrusted() && p .GetDepthInMainChain() > 0) {
                ret += it->second.tx->vout.at( .n).nValue;
            }
        }
    }
    return ret;
...
{
    return loopTxsBalance([filter](const uint256& id, const CWalletTx& p , CAmount& nTotal) {
            if (!p .IsTrusted() && p .GetDepthInMainChain() == 0 && p .InMempool())
                nTotal += p .GetCredit(filter);
    });
}
...
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& p , CAmount& nTotal) {
            nTotal += p .GetImmatureCredit(false);
    });
}
...
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& p , CAmount& nTotal) {
            nTotal += p .GetImmatureCredit(false, ISMINE_COLD);
    });
}
...
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& p , CAmount& nTotal) {
            nTotal += p .GetImmatureCredit(false, ISMINE_SPENDABLE_DELEGATED);
    });
}
...
CAmount CWallet::GetWatchOnlyBalance() const
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& p , CAmount& nTotal) {
            if (p .IsTrusted())
                nTotal += p .GetAvailableWatchOnlyCredit();
    });
}
...
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& p , CAmount& nTotal) {
            nTotal += p .GetImmatureWatchOnlyCredit();
    });
}
...
void CWallet::GetAvailableP2CS s(std::vector<COutput>& v s) const {
    v s.clear();
    {
        LOCK(cs_wallet);
        for (const auto& it : mapWallet) {
            ...
            bool fConflicted;
            int nDepth = p ->GetDepthAndMempool(fConflicted);

            if (fConflicted || nDepth < 0)
                continue;

            bool fSafe = p ->IsTrusted();

            if (p ->tx->HasP2CSOutputs()) {
                for (int i = 0; i < (int) p ->tx->vout.size(); i++) {
                    const auto &utxo = p ->tx->vout[i];
                    ...
                    if (utxo.scriptPubKey.IsPayToColdStaking()) {
                        ...
                        if (mine & ISMINE_COLD || isMineSpendable)
                            ...
                            v s.emplace_back(p , i, 0, isMineSpendable, true, fSafe);
                    }
                }
            }
        }
 ...
 */
static bool CheckTXAvailabilityInternal(const CWalletTx* p , bool fOnlySafe, int& nDepth, bool& safeTx)
{
    safeTx = p ->IsTrusted();
    if (fOnlySafe && !safeTx) return false;
    if (p ->GetBlocksToMaturity() > 0) return false;

    nDepth = p ->GetDepthInMainChain();

    // We should not consider  s which aren't at least in our mempool
    // It's possible for these to be conflicted via ancestors which we may never be able to detect
    if (nDepth == 0 && !p ->InMempool()) return false;

    return true;
}

// cs_main lock required
static bool CheckTXAvailability(const CWalletTx* p , bool fOnlySafe, int& nDepth, bool& safeTx)
{
    AssertLockHeld(cs_main);
    if (!CheckFinalTx(p ->tx)) return false;
    return CheckTXAvailabilityInternal(p , fOnlySafe, nDepth, safeTx);
}

// cs_main lock NOT required
static bool CheckTXAvailability(const CWalletTx* p ,
                         bool fOnlySafe,
                         int& nDepth,
                         bool& safeTx,
                         int nBlockHeight)
{
    ...
    if (!IsFinalTx(p ->tx, nBlockHeight + 1, GetAdjustedTime())) return false;
    return CheckTXAvailabilityInternal(p , fOnlySafe, nDepth, safeTx);
}
...
bool CWallet::GetMasternodeVinAndKeys(CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash, std::string strOutputIndex, std::string& strError)
{
    ...
    if (txOut.nValue != Params().GetConsensus().nMNCollateralAmt) {
        strError = strprintf("Invalid collateral tx value, must be %s clbi", FormatMoney(Params().GetConsensus().nMNCollateralAmt));
        ...
        const bool fIncludeLocked) const
{
    ...
    // Skip locked utxo
    if (!fIncludeLocked && IsLocked (wtxid, outIndex)) return res;

    // Check if we should include zero value utxo
    if (output.nValue <= 0) return res;
    if (f sSelected &&  Control && ! Control->fAllowOtherInputs && ! Control->IsSelected(COutPoint(wtxid, outIndex)))
        return res;
    ...
    res.spendable = ((mine & ISMINE_SPENDABLE) != ISMINE_NO) ||
                     (((mine & ISMINE_WATCH_ONLY) != ISMINE_NO) && ( Control &&  Control->fAllowWatchOnly && res.solvable)) ||
...
bool CWallet::Available s(std::vector<COutput>* p s,      // --> populates when != nullptr
                             const C Control*  Control,   // Default: nullptr
                             Available sFilter  sFilter) const
{
    if (p s) p s->clear();
    const bool f sSelected = ( Control != nullptr) &&  Control->HasSelected();
    // include delegated  s when  Control is active
    if (! sFilter.fIncludeDelegated && f sSelected)
         sFilter.fIncludeDelegated = true;

    {
        ...
        for (const auto& entry : mapWallet) {
            ...
            const CWalletTx* p  = &entry.second;

            // Check if the tx is selectable
            int nDepth = 0;
            bool safeTx = false;
            if (!CheckTXAvailability(p ,  sFilter.fOnlySafe, nDepth, safeTx, m_last_block_processed_height))
                continue;

            // Check min depth filtering requirements
            if (nDepth <  sFilter.minDepth) continue;

            for (unsigned int i = 0; i < p ->tx->vout.size(); i++) {
                const auto& output = p ->tx->vout[i];

                // Filter by value if needed
                if ( sFilter.nMaxOutValue > 0 && output.nValue >  sFilter.nMaxOutValue) {
                    continue;
                }
                if ( sFilter.nMinOutValue > 0 && output.nValue <  sFilter.nMinOutValue) {
                    continue;
                }

                // Filter by specific destinations if needed
                if ( sFilter.onlyFilteredDest && ! sFilter.onlyFilteredDest->empty()) {
                    CTxDestination address;
                    if (!ExtractDestination(output.scriptPubKey, address) || ! sFilter.onlyFilteredDest->count(address)) {
                        continue;
                    }
                }
                ...
                auto res = CheckOutputAvailability(
                        ...
                        wtxid,
                         Control,
                        f sSelected,
                         sFilter.fIncludeColdStaking,
                         sFilter.fIncludeDelegated,
                         sFilter.fIncludeLocked);

                if (!res.available) continue;
                if ( sFilter.fOnlySpendable && !res.spendable) continue;

                // found valid  
                if (!p s) return true;
                p s->emplace_back(p , (int) i, nDepth, res.spendable, res.solvable, safeTx);

                // Checks the sum amount of all UTXO's.
                if ( sFilter.nMinimumSumAmount != 0) {
                    nTotal += output.nValue;

                    if (nTotal >=  sFilter.nMinimumSumAmount) {
                        return true;
                    }
                }

                // Checks the maximum number of UTXO's.
                if ( sFilter.nMaximumCount > 0 && p s->size() >=  sFilter.nMaximumCount) {
                    return true;
                }
            }
        }
        return (p s && !p s->empty());
    }
}

std::map<CTxDestination , std::vector<COutput> > CWallet::Available sByAddress(bool fOnlySafe, CAmount max Value, bool fIncludeColdStaking)
{
    CWallet::Available sFilter  Filter;
     Filter.fIncludeColdStaking = true;
     Filter.fOnlySafe = fOnlySafe;
     Filter.fIncludeColdStaking = fIncludeColdStaking;
     Filter.nMaxOutValue = max Value;
    std::vector<COutput> v s;
    Available s(&v s, nullptr,  Filter);

    std::map<CTxDestination, std::vector<COutput> > map s;
    for (const COutput& out : v s) {
        CTxDestination address;
        ...
        if (!ExtractDestination(out.tx->tx->vout[out.i].scriptPubKey, address, fColdStakeAddr)) {
            ...
            if (isP2CS && !fIncludeColdStaking) {
                // It must never happen as the   filtering process shouldn't had added the P2CS in the first place
                assert(false);
            }
        ...
        map s[address].emplace_back(out);
    }
...
bool CWallet::Stakeable s(std::vector<CStakeableOutput>* p s)
{
    ...
    for (const auto& it : mapWallet) {
        ...
        int nDepth = 0;
        bool safeTx = false;
        if (!CheckTXAvailability(p , true, nDepth, safeTx))
            continue;
        ...
        for (unsigned int index = 0; index < p ->tx->vout.size(); index++) {
            ...
            // found valid  
            if (!p s) return true;
            if (!pindex) pindex = mapBlockIndex.at(p ->m_confirm.hashBlock);
            p s->emplace_back(p , (int) index, nDepth, pindex);
        }
    }
    return (p s && !p s->empty());
}

bool CWallet::Select sMinConf(const CAmount& nTargetValue, int nConfMine, int nConfTheirs, uint64_t nMaxAncestors, std::vector<COutput> v s, std::set<std::pair<const CWalletTx*, unsigned int> >& set sRet, CAmount& nValueRet) const
{
    set sRet.clear();
    nValueRet = 0;

    // List of values less than target
    std::pair<CAmount, std::pair<const CWalletTx*, unsigned int> >  LowestLarger;
     LowestLarger.first = std::numeric_limits<CAmount>::max();
     LowestLarger.second.first = NULL;
    std::vector<std::pair<CAmount, std::pair<const CWalletTx*, unsigned int> > > vValue;
    CAmount nTotalLower = 0;

    Shuffle(v s.begin(), v s.end(), FastRandomContext());

    for (const COutput& output : v s) {
        if (!output.fSpendable)
            continue;

        const CWalletTx* p  = output.tx;

        if (output.nDepth < (p ->IsFromMe(ISMINE_ALL) ? nConfMine : nConfTheirs)) {
            continue;
        }

        if (!mempool.TransactionWithinChainLimit(p ->GetHash(), nMaxAncestors)) {
            continue;
        }

        int i = output.i;
        CAmount n = p ->tx->vout[i].nValue;

        std::pair<CAmount, std::pair<const CWalletTx*, unsigned int> >   = std::make_pair(n, std::make_pair(p , i));

        if (n == nTargetValue) {
            set sRet.insert( .second);
            nValueRet +=  .first;
            return true;
        } else if (n < nTargetValue + MIN_CHANGE) {
            vValue.push_back( );
            nTotalLower += n;
        } else if (n <  LowestLarger.first) {
             LowestLarger =  ;
        }
    }
    ...
    if (nTotalLower == nTargetValue) {
        for (unsigned int i = 0; i < vValue.size(); ++i) {
            set sRet.insert(vValue[i].second);
    ...
    if (nTotalLower < nTargetValue) {
        if ( LowestLarger.second.first == NULL)
                return false;
        set sRet.insert( LowestLarger.second);
        nValueRet +=  LowestLarger.first;
        return true;
    }
    ...
    // If we have a bigger   and (either the stochastic approximation didn't find a good solution,
    //                                   or the next bigger   is closer), return the bigger  
    if ( LowestLarger.second.first &&
        ((nBest != nTargetValue && nBest < nTargetValue + MIN_CHANGE) ||  LowestLarger.first <= nBest)) {
        set sRet.insert( LowestLarger.second);
        nValueRet +=  LowestLarger.first;
    } else {
        std::string s = "CWallet::Select sMinConf best subset: ";
        for (unsigned int i = 0; i < vValue.size(); i++) {
            if (vfBest[i]) {
                set sRet.insert(vValue[i].second);
                nValueRet += vValue[i].first;
...
bool CWallet::Select sToSpend(const std::vector<COutput>& vAvailable s, const CAmount& nTargetValue, std::set<std::pair<const CWalletTx*, unsigned int> >& set sRet, CAmount& nValueRet, const C Control*  Control) const
{
    // Note: this function should never be used for "always free" tx types like dstx
    std::vector<COutput> v s(vAvailable s);

    //   control -> return all selected outputs (we want all selected to go into the transaction for sure)
    if ( Control &&  Control->HasSelected() && ! Control->fAllowOtherInputs) {
        for (const COutput& out : v s) {
            if (!out.fSpendable)
                continue;

            nValueRet += out.tx->tx->vout[out.i].nValue;
            set sRet.emplace(out.tx, out.i);
        }
        return (nValueRet >= nTargetValue);
    }

    // calculate value from preset inputs and store them
    std::set<std::pair<const CWalletTx*, uint32_t> > setPreset s;
    CAmount nValueFromPresetInputs = 0;

    std::vector<OutPointWrapper> vPresetInputs;
    if ( Control)
         Control->ListSelected(vPresetInputs);
    for (const auto& outpoint : vPresetInputs) {
        ...
        if (it != mapWallet.end()) {
            ...
            if (p ->tx->vout.size() <= outpoint.outPoint.n)
                return false;
            nValueFromPresetInputs += p ->tx->vout[outpoint.outPoint.n].nValue;
            setPreset s.emplace(p , outpoint.outPoint.n);
        } else
            return false; // TODO: Allow non-wallet inputs
    }

    // remove preset inputs from v s
    for (std::vector<COutput>::iterator it = v s.begin(); it != v s.end() &&  Control &&  Control->HasSelected();) {
        if (setPreset s.count(std::make_pair(it->tx, it->i)))
            it = v s.erase(it);
        else
            ++it;
    }
    ...
    bool res = nTargetValue <= nValueFromPresetInputs ||
            Select sMinConf(nTargetValue - nValueFromPresetInputs, 1, 6, 0, v s, set sRet, nValueRet) ||
            Select sMinConf(nTargetValue - nValueFromPresetInputs, 1, 1, 0, v s, set sRet, nValueRet) ||
            (bSpendZeroConfChange && Select sMinConf(nTargetValue - nValueFromPresetInputs, 0, 1, 2, v s, set sRet, nValueRet)) ||
            (bSpendZeroConfChange && Select sMinConf(nTargetValue - nValueFromPresetInputs, 0, 1, std::min((size_t)4, nMaxChainLength/3), v s, set sRet, nValueRet)) ||
            (bSpendZeroConfChange && Select sMinConf(nTargetValue - nValueFromPresetInputs, 0, 1, nMaxChainLength/2, v s, set sRet, nValueRet)) ||
            (bSpendZeroConfChange && Select sMinConf(nTargetValue - nValueFromPresetInputs, 0, 1, nMaxChainLength, v s, set sRet, nValueRet));

    // because Select sMinConf clears the set sRet, we now add the possible inputs to the  set
    set sRet.insert(setPreset s.begin(), setPreset s.end());
...
std::map<std::pair<CTxDestination, Optional<CTxDestination>>, std::vector<COutput>> CWallet::List s() const
{
    ...
    std::vector<COutput> available s;
    Available s(&available s, nullptr, filter);

    for (const COutput&   : available s) {
        const CScript& scriptPubKey =  .tx->tx->vout[ .i].scriptPubKey;
        ...
        if (ExtractDestinations(scriptPubKey, type, addresses, nRequired)) {
            if (addresses.size() == 1) {
                ...
                result[addrpair].emplace_back(std::move( ));
...
bool CWallet::CreateBudgetFeeTX(CTransactionRef& tx, const uint256& hash, CReserveKey& keyChange, bool fFinalization)
{
    ...
    C Control*  Control = nullptr;
    int nChangePosInOut = -1;
    bool success = CreateTransaction(vecSend, tx, keyChange, nFeeRet, nChangePosInOut, strFail,  Control, true, (CAmount)0);
    if (!success) {
        LogPrintf("%s: Error - %s\n", __func__, strFail);
        return false;
    }
...
bool CWallet::FundTransaction(CMutableTransaction& tx, CAmount& nFeeRet, bool overrideEstimatedFeeRate, const CFeeRate& specificFeeRate, int& nChangePosInOut, std::string& strFailReason, bool includeWatching, bool lockUnspents, const std::set<int>& setSubtractFeeFromOutputs, const CTxDestination& destChange)
{
    ...
    C Control  Control;
     Control.destChange = destChange;
     Control.fAllowOtherInputs = true;
     Control.fAllowWatchOnly = includeWatching;
     Control.fOverrideFeeRate = overrideEstimatedFeeRate;
     Control.nFeeRate = specificFeeRate;

    const int nExtraSize = tx.isSaplingVersion() ?
            (int)(GetSerializeSize(tx.sapData) + GetSerializeSize(tx.extraPayload)) : 0;

    for (const CTxIn& txin : tx.vin) {
         Control.Select(txin.prevout);
    }

    // Acquire the locks to prevent races to the new locked unspents between the
    // CreateTransaction call and Lock  calls (when lockUnspents is true).
    LOCK2(cs_main, cs_wallet);

    CReserveKey reservekey(this);
    CTransactionRef wtx;
    if (!CreateTransaction(vecSend, wtx, reservekey, nFeeRet, nChangePosInOut, strFailReason, & Control, false, 0, false, nullptr, nExtraSize)) {
        return false;
    }
    ...
    for (const CTxIn& txin : wtx->vin) {
        if (! Control.IsSelected(txin.prevout)) {
            tx.vin.emplace_back(txin);
            if (lockUnspents) Lock (txin.prevout);
        }
    }
...
std::vector<COutput> CWallet::GetOutputsFrom Control(const C Control*  Control)
{
    assert( Control);
    LOCK(cs_wallet);
    std::vector<COutput> v sRet;
    std::vector<OutPointWrapper> vPresetInputs;
     Control->ListSelected(vPresetInputs);
    for (const auto& out : vPresetInputs) {
        ...
        if (it != mapWallet.end()) {
            ...
            v sRet.emplace_back(COutput(&(it->second), out.outPoint.n, 0, true, true, true));
        }
    }
    return v sRet;
}
    ...
    int nMinDepth)
{
    ...
    CWallet::Available sFilter  Filter;
     Filter.fOnlySpendable = true;
     Filter.fIncludeDelegated = fIncludeDelegated;
     Filter.minDepth = nMinDepth;

    {
        std::set<std::pair<const CWalletTx*,unsigned int> > set s;
        LOCK2(cs_main, cs_wallet);
        {
            std::vector<COutput> vAvailable s;
            if ( Control &&  Control->HasSelected() && ! Control->fAllowOtherInputs) {
                // Select only the outputs that the caller pre-selected.
                vAvailable s = GetOutputsFrom Control( Control);
            } else {
                // Regular selection
                Available s(&vAvailable s,  Control,  Filter);
            }
            ...
            while (true) {
                ...
                if (!Select sToSpend(vAvailable s, nValueToSelect, set s, nValueIn,  Control)) {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-clbi-address
                    bool combineChange = false;

                    //   control: send change to custom address
                    if ( Control && IsValidDestination( Control->destChange)) {
                        scriptChange = GetScriptForDestination( Control->destChange);
                    ...
                    // no   control: send change to newly generated address
                    else {
                ...
                // Fill vin
                for (const std::pair<const CWalletTx*, unsigned int>&   : set s) {
                    if(fStakeDelegationVoided &&  .first->tx->vout[ .second].scriptPubKey.IsPayToColdStaking()) {
                        *fStakeDelegationVoided = true;
                    }
                    txNew.vin.emplace_back( .first->GetHash(),  .second);
                }

                // Fill in dummy signatures for fee calculation.
                int nIn = 0;
                for (const auto &   : set s) {
                    const CScript& scriptPubKey =  .first->tx->vout[ .second].scriptPubKey;
                ...
                if ( Control && nFeeNeeded > 0 &&  Control->nMinimumTotalFee > nFeeNeeded) {
                    nFeeNeeded =  Control->nMinimumTotalFee;
                }
                if ( Control &&  Control->fOverrideFeeRate)
                    nFeeNeeded =  Control->nFeeRate.GetFee(nBytes);
        ...
        if (sign) {
            ...
            for (const auto&   : set s) {
                const CScript& scriptPubKey =  .first->tx->vout[ .second].scriptPubKey;
                SignatureData sigdata;
                bool haveKey =  .first->GetStakeDelegationCredit() > 0;

                if (!ProduceSignature(
                        TransactionSignatureCreator(this, &txNewConst, nIn,  .first->tx->vout[ .second].nValue, SIGHASH_ALL),
                        scriptPubKey,
                        sigdata,
...
bool CWallet::CreateTransaction(CScript scriptPubKey, const CAmount& nValue, CTransactionRef& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, std::string& strFailReason, const C Control*  Control, CAmount nFeePay, bool fIncludeDelegated, bool* fStakeDelegationVoided, int nExtraSize, int nMinDepth)
{
    std::vector<CRecipient> vecSend;
    vecSend.emplace_back(scriptPubKey, nValue, false);
    int nChangePosInOut = -1;
    return CreateTransaction(vecSend, wtxNew, reservekey, nFeeRet, nChangePosInOut, strFailReason,  Control, true, nFeePay, fIncludeDelegated, fStakeDelegationVoided, nExtraSize, nMinDepth);
}
...
bool CWallet::Create Stake(
        const CBlockIndex* pindexPrev,
        unsigned int nBits,
        CMutableTransaction& txNew,
        int64_t& nTxNewTime,
        std::vector<CStakeableOutput>* available s,
        bool stopOnNewBlock) const
{
    // shuffle  s
    if (available s && Params().IsRegmainet()) {
        Shuffle(available s->begin(), available s->end(), FastRandomContext());
    }

    // Mark   stake transaction
    txNew.vin.clear();
    txNew.vout.clear();
    ...
    pStakerStatus->SetLast s((int) available s->size());
    ...
    int nAttempts = 0;
    for (auto it = available s->begin(); it != available s->end();) {
        COutPoint outPoint = COutPoint(it->tx->GetHash(), it->i);
        CclbiStake stakeInput(it->tx->tx->vout[it->i],
                             outPoint,
                             it->pindex);
        ...
        if (WITH_LOCK(cs_wallet, return IsSpent(outPoint))) {
            // remove it from the available  s
            it = available s->erase(it);
            continue;
        }
        ...
        // Found a kernel
        LogPrintf("Create Stake : kernel found\n");
        ...
        if (nBytes >= DEFAULT_BLOCK_MAX_SIZE / 5)
            return error("%s : exceeded  stake size limit", __func__);
...
bool CWallet::Sign Stake(CMutableTransaction& txNew) const
{
    // Sign it
    int nIn = 0;
    for (const CTxIn& txIn : txNew.vin) {
        ...
        if (!wtx || !SignSignature(*this, *(wtx->tx), txNew, nIn++, SIGHASH_ALL, true))
            return error("%s : failed to sign  stake", __func__);
    }

    // Successfully signed  stake
    return true;
}
...
std::map<CTxDestination, CAmount> CWallet::GetAddressBalances()
{
...

    {
        ...
        for (const auto& walletEntry : mapWallet) {
            const CWalletTx* p  = &walletEntry.second;

            if (!IsFinalTx(p ->tx, m_last_block_processed_height) || !p ->IsTrusted())
                continue;

            if (p ->Is Base() && p ->GetBlocksToMaturity() > 0)
                continue;
            ...
            if (fConflicted)
                continue;
            if (nDepth < (p ->IsFromMe(ISMINE_ALL) ? 0 : 1))
                continue;

            for (unsigned int i = 0; i < p ->tx->vout.size(); i++) {
                CTxDestination addr;
                if (!IsMine(p ->tx->vout[i]))
                    continue;
                if ( !ExtractDestination(p ->tx->vout[i].scriptPubKey, addr) &&
                        !ExtractDestination(p ->tx->vout[i].scriptPubKey, addr, true) )
                    continue;

                CAmount n = IsSpent(walletEntry.first, i) ? 0 : p ->tx->vout[i].nValue;
...
std::set<std::set<CTxDestination> > CWallet::GetAddressGroupings()
{
    ...
    for (const auto& walletEntry : mapWallet) {
        ...
        for (unsigned int i = 0; i < p ->tx->vout.size(); i++)
            if (IsMine(p ->tx->vout[i])) {
                CTxDestination address;
                if (!ExtractDestination(p ->tx->vout[i].scriptPubKey, address))
                    continue;
                grouping.insert(address);
                groupings.insert(grouping);
                grouping.clear();
...
void CWallet::Lock (const COutPoint& output)
{
    AssertLockHeld(cs_wallet); // setLocked s
    setLocked s.insert(output);
}

void CWallet::Unlock (const COutPoint& output)
{
    AssertLockHeld(cs_wallet); // setLocked s
    setLocked s.erase(output);
}

void CWallet::UnlockAll s()
{
    AssertLockHeld(cs_wallet); // setLocked s
    setLocked s.clear();
}

bool CWallet::IsLocked (const uint256& hash, unsigned int n) const
{
    AssertLockHeld(cs_wallet); // setLocked s
    const COutPoint outpt(hash, n);

    return (setLocked s.count(outpt) > 0);
}

std::set<COutPoint> CWallet::ListLocked s()
{
    AssertLockHeld(cs_wallet);
    return setLocked s;
}
...
void CWallet::AutoCombineDust(CConnman* connman)
{
    ...
    std::map<CTxDestination, std::vector<COutput> > map sByAddress =
            Available sByAddress(true, nAutoCombineThreshold, false);

    // s are sectioned by address. This combination code only wants to combine inputs that belong to the same address
    for (const auto& entry : map sByAddress) {
        std::vector<COutput> v s, vReward s;
        bool maxSize = false;
        v s = entry.second;
        ...
        C Control*  Control = new C Control();
        ...
        if (! Control->HasSelected())
            continue;

        //we cannot combine one   with itself
        if (vReward s.size() <= 1)
            continue;
        ...
        if (!ExtractDestination(scriptPubKey, destMyAddress)) {
         ...
         Control->destChange = destMyAddress;

        // Create the transaction and commit it to the network
        CTransactionRef wtx;
        CReserveKey keyChange(this); // this change address does not end up being used, because change is returned with   control switch
        std::string strErr;
        CAmount nFeeRet = 0;
        int nChangePosInOut = -1;
...

        {
            ...
            LOCK2(cs_main, cs_wallet);
            if (!CreateTransaction(vecSend, wtx, keyChange, nFeeRet, nChangePosInOut, strErr,  Control,
...
CWallet* CWallet::CreateWalletFromFile(const std::string& name, const fs::path& path)
{
    ...
    if (gArgs.GetBoolArg("-upgradewallet", fFirstRun && !fLegacyWallet) ||
        ...
        if (prev_version <= FEATURE_PRE_clbi && walletInstance->IsLocked()) {
...
int CWalletTx::GetBlocksToMaturity() const
{
    if (!(Is Base() || Is Stake()))
        return 0;
    return std::max(0, (Params().GetConsensus().n baseMaturity + 1) - GetDepthInMainChain());
}

bool CWalletTx::IsInMainChainImmature() const
{
    if (!Is Base() && !Is Stake()) return false;
    const int depth = GetDepthInMainChain();
    return (depth > 0 && depth <= Params().GetConsensus().n baseMaturity);
}
