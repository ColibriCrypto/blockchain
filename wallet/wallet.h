...
#ifndef clbi_WALLET_H
#define clbi_WALLET_H
...
static const CAmount nHighTransactionFeeWarning = 0.1 *  ;
//! -mintxfee default
static const CAmount DEFAULT_TRANSACTION_MINFEE = 10000;
//! -maxtxfee default
static const CAmount DEFAULT_TRANSACTION_MAXFEE = 1 *  ;
...
static const CAmount DEFAULT_MIN_STAKE_SPLIT_THRESHOLD = 100 *  ;
...
enum WalletFeature {
    ...
    FEATURE_PRE_clbi = 61000, // inherited version..

    // The following features were implemented in clbi but not in our wallet, we can simply skip them.
 ...
 *  - n s         number of stakeable utxos during last attempt
**/
class CStakerStatus
{
private:
...
public:
    ...
    bool Is Base() const { return tx->Is Base(); }
    bool Is Stake() const { return tx->Is Stake(); }
...
private:
    ...
    */
    ...
    std::vector<COutput> GetOutputsFrom Control(const C Control*  Control);
...
public:
    ...
    struct Available sFilter {
        public:
        Available sFilter() {}
        Available sFilter(bool _fIncludeDelegated,
    ...
    /**
     * populate v s with vector of available COutputs.
     */
    bool Available s(std::vector<COutput>* p s,   // --> populates when != nullptr
                        const C Control*  Control = nullptr,
                        Available sFilter  sFilter = Available sFilter()
                        ) const;
    //! >> Available  s (spending)
    bool Select sToSpend(const std::vector<COutput>& vAvailable s, const CAmount& nTargetValue, std::set<std::pair<const CWalletTx*, unsigned int> >& set sRet, CAmount& nValueRet, const C Control*  Control = nullptr) const;

    /**
     * Select  s until nTargetValue is reached. Return the actual value
     * and the corresponding   set.
     */
    bool Select sMinConf(const CAmount& nTargetValue, int nConfMine, int nConfTheirs, uint64_t nMaxAncestors, std::vector<COutput> v s, std::set<std::pair<const CWalletTx*, unsigned int> >& set sRet, CAmount& nValueRet) const;
    //! >> Available  s (staking)
    bool Stakeable s(std::vector<CStakeableOutput>* p s = nullptr);
    //! >> Available  s (P2CS)
    void GetAvailableP2CS s(std::vector<COutput>& v s) const;

    std::map<CTxDestination, std::vector<COutput> > Available sByAddress(bool fConfirmed, CAmount max Value, bool fIncludeColdStaking);

    /**
     * Return list of available  s and locked  s grouped by non-change output address.
     * clbi: group  s by pair <CTxDestination, Optional<CTxDestination>>. The optional destination
     * is reserved for the staker address in case of P2CS.
     */
    std::map<std::pair<CTxDestination, Optional<CTxDestination>>, std::vector<COutput>> List s() const;
    /**
    ...
    /// Get 10000 clbi output and keys which can be used for the Masternode
    ...
    bool IsLocked (const uint256& hash, unsigned int n) const;
    void Lock (const COutPoint& output);
    void Unlock (const COutPoint& output);
    void UnlockAll s();
    std::set<COutPoint> ListLocked s();
     ...
     *  Lock for spending the   c, if it's owned by the wallet, it's unspent, and:
     ...
     *  lock the corresponding  , to prevent accidental spending.
     */
    ...
    struct Balance {
        ...
        CAmount m_mine_immature{0};              //!< Immature  bases/ stakes in the main chain
    ...
    CAmount GetColdStakingBalance() const;  // delegated  s for which we have the staking key
    CAmount GetImmatureColdStakingBalance() const;
    CAmount GetStakingBalance(const bool fIncludeColdStaking = true) const;
    CAmount GetDelegatedBalance() const;    // delegated  s for which we have the spending key
    CAmount GetImmatureDelegatedBalance() const;
    CAmount GetLocked s() const;
    ...
    /**
     * Create a new transaction paying the recipients with a set of  s
     * selected by Select s(); Also create the change output, when needed
    ...
    bool CreateTransaction(CScript scriptPubKey, const CAmount& nValue, CTransactionRef& tx, CReserveKey& reservekey, CAmount& nFeeRet, std::string& strFailReason, const C Control*  Control = nullptr, CAmount nFeePay = 0, bool fIncludeDelegated = false, bool* fStakeDelegationVoided = nullptr, int nExtraSize = 0, int nMinDepth = 0);
    ...
    bool Create Stake(const CBlockIndex* pindexPrev,
                         unsigned int nBits,
                         CMutableTransaction& txNew,
                         int64_t& nTxNewTime,
                         std::vector<CStakeableOutput>* available s,
                         bool stopOnNewBlock = true) const;
    bool Sign Stake(CMutableTransaction& txNew) const;
