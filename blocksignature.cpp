...
bool CheckBlockSignature(const CBlock& block)
{
    ...
    /** Each block is signed by the private key of the input that is staked. This can be either zclbi or normal UTXO
     *  zclbi: Each zclbi has a keypair associated with it. The serial number is a hash of the public key.
     *  UTXO: The public key that signs must match the public key associated with the first utxo of the  stake tx.
     */
    CPubKey pubkey;
    bool fzclbiStake = block.vtx[1]->vin[0].IsZero Spend();
    if (fzclbiStake) {
        libzero :: Spend spend = TxInToZero Spend(block.vtx[1]->vin[0]);
