...
* @file       Params.h
*
* @brief      Parameter classes for Zero .
*
* @author       
* @date       June  
*
...
public:
	/** @brief Construct a set of Zero  parameters from a modulus "N".
	* @param N                A trusted RSA modulus
	* @param securityLevel    A security level expressed in symmetric bits (default 80)
	*
	* Allocates and derives a set of Zero  parameters from
	...
	* be known to any party, or the security of Zero  is
	...
	**/
	...
	/**
	 * Lower bound on the value for committed  .
	 * Required by the accumulator proof.
	 */
	CBigNum min Value;

	/**
	 * Upper bound on the value for a comitted  .
	 * Required by the accumulator proof.
	 */
	CBigNum max Value;
	 ...
	 * a   (which it self is a commitment to a serial number).
    ...
    SERIALIZE_METHODS(AccumulatorAndProofParams, obj) {
        ...
        READWRITE(obj.accumulatorQRNCommitmentGroup, obj.min Value, obj.max Value, obj.k_prime, obj.k_dprime);
    }
};
...
public:
	...
	* in length. 3072 bits is strongly recommended.
	**/
	Zero Params(CBigNum accumulatorModulus,
	       uint32_t securityLevel = ZERO _DEFAULT_SECURITYLEVEL);
	 ...
	 * a   as a commitment  to a serial number.
	 */
	IntegerGroupParams  CommitmentGroup;

	/**
	 * One of two groups used to form a commitment to
	 * a   (which it self is a commitment to a serial number).
	 * This is the one used in the serial number poof.
	 * It's order must be equal to the modulus of  CommitmentGroup.
	 */
    ...
    SERIALIZE_METHODS(Zero Params, obj) {
        READWRITE(obj.initialized, obj.accumulatorParams, obj. CommitmentGroup, obj.serialNumberSoKCommitmentGroup, obj.zkp_iterations, obj.zkp_hash_len);
    }
};
