...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    case MSG_TX: {
        ...
        return recentRejects->contains(inv.hash) ||
               ...
               p sTip->Have InCache(COutPoint(inv.hash, 0)) || // Best effort: only try output 0 and 1
               p sTip->Have InCache(COutPoint(inv.hash, 1));
    }
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived, CConnman* connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // clbi: We use certain sporks during IBD, so check to see if they are
    ...
    if (strCommand == NetMsgType::VERACK) {
        ...
        if (ptx->ContainsZero s()) {
            // Don't even try to check zero s at all.
    ...
    else if (strCommand == NetMsgType::HEADERS && Params().HeadersFirstSyncingActive() && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        for (const CBlockHeader& header : headers) {
             ...
             * before headers are reimplemented on  
             */
...
{
    {
        ...
        if (!state.fSyncStarted && !pto->fClient && !fImporting && !fReindex) {
            ...
            if ((nSyncStarted == 0 && fFetch) || pindexBestHeader->GetBlockTime() > GetAdjustedTime() - 6 * 60 * 60) { // NOTE: was "close to today" and 24h in CLBI
