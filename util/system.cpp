...
const char * const clbi_CONF_FILENAME = "clbi.conf";
const char * const clbi_PID_FILENAME = "clbi.pid";
const char * const clbi_MASTERNODE_CONF_FILENAME = "masternode.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager& am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "clbid -foo=bar
    ...
    /* Special test for -  and -regtest args, because we
     * don't want to be confused by craziness like "[regtest]  =1"
     */
...
ArgsManager::ArgsManager() :
     ...
     *   were used while running on regtest/  (or vice-versa).
     * Setting them as section_only_args ensures that sharing a config file
     * between   and regtest/  won't cause problems due to these
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\clbi
// Windows >= Vista: C:\Users\Username\AppData\Roaming\clbi
// Mac: ~/Library/Application Support/clbi
// Unix: ~/.clbi
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "clbi";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\clbiParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\clbiParams
    // Mac: ~/Library/Application Support/clbiParams
    // Unix: ~/.clbi-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "clbiParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectories(pathRet);
    return pathRet / "clbiParams";
#else
    // Unix
    return pathRet / ".clbi-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/clbi/sapling-spend.params";
        sapling_output = "/usr/share/clbi/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/clbi/sapling-spend.params";
            sapling_output = "/usr/local/share/clbi/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", clbi_MASTERNODE_CONF_FILENAME));
...
std::string ArgsManager::GetChainName() const
{
    ...
    bool fmainet = ArgsManagerHelper::GetNetBoolArg(*this, "- ");

    if (fmainet && fRegTest)
        throw std::runtime_error("Invalid combination of -regtest and - .");
    if (fRegTest)
        return CBaseChainParams::REGTEST;
    if (fmainet)
        return CBaseChainParams:: ;
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", clbi_PID_FILENAME));
